...
from .driver import PlaywrightTowerDriver, TowerDriverFactory
from .parser import (
    PageContractError,
...
class ChinaTowerEprocAdapter(PlatformAdapter):
    """Experimental public-announcement adapter for China Tower e-procurement."""

    def __init__(
        self,
        descriptor: PlatformDescriptor,
        *,
        driver_factory: TowerDriverFactory | None = None,
        headless: bool = True,
        timeout_ms: int = 45_000,
        user_data_dir: Path | None = None,
    ) -> None:
        self.descriptor = descriptor
        self.profile = CHINA_TOWER_EPROC_PROFILE
        self.driver_factory = driver_factory or (
            lambda: PlaywrightTowerDriver(
                self.profile,
                headless=headless,
    ...
    async def execute(
        ...
        try:
            async with self.driver_factory() as driver:
                ...
                ranked = [
                    (*match_score(request.query_name, entry.title), entry)
                ...
                if not selected:
                    ...
                    raise PageContractError(
                        "Search completed without verified detail links or a no-result marker"
                    )
                ...
                return self._result(
                    ...
                    artifacts=artifacts,
                    hits=hits,
                    feedback=f"Collected {len(hits)} verified public detail pages.",
                    retryable=False,
                )
