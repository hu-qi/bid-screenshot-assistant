...
from .parser import PageContractError, is_unhydrated_or_error, normalized_text


class UnicomBrowserDriver(Protocol):
    async def search(self, query_name: str) -> BrowserSnapshot: ...

    async def fetch_detail(self, url: str) -> BrowserSnapshot: ...


UnicomDriverFactory = Callable[[], AbstractAsyncContextManager[UnicomBrowserDriver]]


class PlaywrightUnicomDriver:
    def __init__(
        self,
    ...
    async def __aenter__(self) -> PlaywrightUnicomDriver:
        try:
    ...
    async def search(self, query_name: str) -> BrowserSnapshot:
        ...
        try:
            ...
            if is_unhydrated_or_error(html):
                raise PageContractError(
                    "Unicom list page did not hydrate or returned error 501"
                ) from exc
            raise PageContractError("Unicom search input is not visible") from exc
        ...
        if is_unhydrated_or_error(list_text):
            raise PageContractError("Unicom list page returned an unhydrated/error state")

        await search_input.fill(query_name)
        await page.get_by_role("button", name=re.compile(r"搜\s*索")).first.click()
        await self._wait_after_navigation(page)
    ...
    async def fetch_detail(self, url: str) -> BrowserSnapshot:
        ...
        try:
            ...
            if is_unhydrated_or_error(body_text):
                raise PageContractError("Unicom detail page did not hydrate")
